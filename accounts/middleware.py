...
from .managers import set_current_tenant, TenantCacheManager
...
class TenantAccessControlMiddleware:
    ...
    def __call__(self, request):
        ...
        if hasattr(request, 'tenant') and request.tenant and not isinstance(request.user, AnonymousUser):
            if not self._user_can_access_tenant(request.user, request.tenant):
                ...
                raise PermissionDenied("You don't have permission to access this tenant.")
    ...
    def _user_can_access_tenant(self, user, tenant):
        ...
        try:
            ...
            if hasattr(login_user, 'can_access_tenant'):
                return login_user.can_access_tenant(tenant)
